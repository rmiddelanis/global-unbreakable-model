...
nb_weeks = (time.time() - os.stat(the_file).st_mtime) / (
...
df = df.drop([i for i in ["plgp", "unemp", "bashs", "ophe", "axhealth", 'share1_orig'] if i in df.columns],
             axis=1)  ## Drops here the data not used, to avoid it counting as missing data. What are included are:gdp_pc_pp, pop, share1, axfin_p, axfin_r, social_p, social_r, urbanization_rat.
...
guessed_social = pd.read_csv(inputs + "df_social_transfers_statistics.csv", index_col=0)[
...
hfa_oldnew = pd.concat([hfa_newest, hfa_previous, hfa15], axis=1,
                       keys=['new', 'old', "15"])  # this is important to join the list of all countries
...
hfa["prepare_scaleup"] = (hfa["P4-C2"] + hfa["P5-C2"] + hfa[
    "P4-C5"]) / 3 / 5  # q_s in the report, ability to scale up support to to affected population after the disaster, normalized between zero and 1
hfa["finance_pre"] = (1 + hfa[
    "P5-C3"]) / 6  # betwenn 0 and 1	!!!!!!!!!!!!!!!!!!!REMARK: INCONSISTENT WITH THE TECHNICAL PAPER. Q_f=1/2(ratings+P5C3/5)
df[["shew", "prepare_scaleup", "finance_pre"]] = hfa[["shew", "prepare_scaleup", "finance_pre"]]
df[["shew", "prepare_scaleup", "finance_pre"]] = df[["shew", "prepare_scaleup", "finance_pre"]].fillna(
    0)  # assumes no reporting is bad situation (caution! do the fillna after inputing to df to get the largest set of index)
...
ratings_raw.country_in_ratings = ratings_raw.country_in_ratings.str.strip().replace(["Congo"], [
    "Congo, Dem. Rep."])  # The creidt rating sources calls DR Congo just Congo. Here str.strip() is needed to remove any space in the raw data. In the raw data, Congo has some spaces after "o". If not used str.strip(), nothing is replaced.
...
ratings["rating"] = ratings.mean(
    axis=1) / 100  # axis=1 is the average across columns, axis=0 is to average across rows. .mean ignores NaN
...
###Ratings + HFA
df["borrow_abi"] = (df["rating"] + df[
    "finance_pre"]) / 2  # Ability and willingness to improve transfers after the disaster
...
##Capital data
k_data = pd.read_csv(inputs + "capital_data.csv", usecols=["code", "cgdpo", "ck"]).replace(
    {"ROM": "ROU", "ZAR": "COD"}).rename(columns={"cgdpo": "prod_from_k", "ck": "k"})  # Zair is congo
iso_country = pd.read_csv(inputs + "iso3_to_wb_name.csv",
                          index_col="iso3")  # matches names in the dataset with world bank country names
k_data.set_index("code", inplace=True)
k_data["country"] = iso_country["country"]
cond = k_data["country"].isnull()
if cond.sum() > 0:
    warnings.warn(
        "this countries appear to be missing from iso3_to_wb_name.csv: " + " , ".join(k_data.index[cond].values))
...
sids_k = pd.read_csv("intermediate/avg_prod_k_with_gar_for_sids.csv").rename(
    columns={"Unnamed: 0": "country"}).set_index("country")
df = df.fillna(sids_k)
df.dropna().shape
#####
...
pager_code_to_aggcat = replace_with_warning(pager_desc_to_code.Description, pager_description_to_aggregate_category,
...
try:
    fa_guessed_gar = fa_guessed_gar.reset_index().set_index(event_level + ['fa']).drop(['index'], axis=1).stack()
except:
    fa_guessed_gar = fa_guessed_gar.reset_index().set_index(event_level + ['fa']).stack()
...
PEB_wb_deltares_older = pd.read_csv(inputs + "PEB_wb_deltares.csv", skiprows=[0, 1, 2],
                                    usecols=["Country", "Nation-wide"])  # Exposure bias from older WB DELTARES study
PEB_wb_deltares_older["country"] = replace_with_warning(PEB_wb_deltares_older["Country"],
                                                        any_to_wb)  # Replace with warning is used for columns, for index set_index is needed.
df["pe"] = df["pe"].fillna(PEB_wb_deltares_older.set_index("country").drop(["Country"],
                                                                           axis=1).squeeze())  # Completes with bias from previous study when pov maps not available. squeeze is needed or else it's impossible to fillna with a dataframe
if use_avg_pe:
    df["pe"] = df["pe"].fillna(
        wavg(df["pe"], df["pop"]))  # use averaged pe from global data for countries that don't have PE.
else:
    df["pe"].fillna(0)
pe = df.pop("pe")
...
if constant_fa:
    if use_2016_inputs:
        fa_guessed_gar.to_csv(inputs + 'constant_fa.csv', header=True)
    else:
...
hazard_ratios["shew"] = hazard_ratios.shew.unstack("hazard").assign(earthquake=0).stack(
    "hazard").reset_index().set_index(event_level + ["income_cat"])  # shew at 0 for earthquake
...
df["tau_tax"], cat_info["gamma_SP"] = social_to_tx_and_gsp(economy,
                                                           cat_info)  # computes tau tax and gamma_sp from socail_poor and social_nonpoor. CHECKED!
cat_info["k"] = (1 - cat_info["social"]) * cat_info["c"] / ((1 - df["tau_tax"]) * df[
    "avg_prod_k"])  # here k in cat_info has poor and non poor, while that from capital_data.csv has only k, regardless of poor or nonpoor
...
# for apol in [None]:
for apol in [None, ['bbb_complete', 1], ['borrow_abi', 2], 'unif_poor', ['bbb_incl', 1], ['bbb_fast', 1],
             ['bbb_fast', 2], ['bbb_fast', 4], ['bbb_fast', 5], ['bbb_50yrstand', 1]]:
    ...
    if drop_unused_data:
        ...
        df_in = df.drop([i for i in
                         ["social_p", "social_r", "pov_head", "pe", "vp", "vr", "axfin_p", "axfin_r", "rating",
                          "finance_pre"] if i in df.columns], axis=1, errors="ignore").dropna()
    else:
        df_in = df.dropna()
    ...
    fa_guessed_gar.to_csv(intermediate + "/fa_guessed_from_GAR_and_PAGER_shaved" + pol_str + ".csv", encoding="utf-8",
                          header=True)
    pd.DataFrame([vp, vr, v], index=["vp", "vr", "v"]).T.to_csv(
        intermediate + "/v_pr_fromPAGER_shaved_GAR" + pol_str + ".csv", encoding="utf-8", header=True)
