...
fa_threshold = 0.9  # if fa is above this threshold, then it is capped at this value
...
df = pd.read_csv(wb_data_file).set_index(economy)
...
# fa is the fraction of asset affected: \Delta K = f_a * V; with f_a the exposure.
...
fa_guessed_gar = pd.merge(fa_guessed_gar.reset_index(), vulnerability_rich.reset_index(), on=economy)
fa_guessed_gar = pd.merge(fa_guessed_gar.reset_index(), vulnerability_poor.reset_index(), on=economy).drop('index', axis=1)
...
data = pd.read_excel(os.path.join(input_dir, "PEB_flood_povmaps.xlsx"))[
    ["iso", "peb"]].dropna()  # Exposure bias from WB povmaps study
df["pe"] = data.set_index(data.iso.replace(iso3_to_wb)).peb - 1
PEB_wb_deltares_older = pd.read_csv(os.path.join(input_dir, "PEB_wb_deltares.csv"), skiprows=[0, 1, 2],
...
for apol in [None, ['bbb_complete', 1], ['borrow_abi', 2], 'unif_poor', ['bbb_incl', 1], ['bbb_fast', 1],
    ...
    fa_guessed_gar.to_csv(intermediate_dir + "/fa_guessed_from_GAR_and_PAGER_shaved" + pol_str + ".csv",
                          encoding="utf-8",
                          header=True)
    pd.DataFrame([vulnerability_poor, vulnerability_rich, vulnerability_tot], index=["vp", "vr", "v"]).T.to_csv(
        intermediate_dir + "/v_pr_fromPAGER_shaved_GAR" + pol_str + ".csv", encoding="utf-8", header=True)
    df_in.to_csv(intermediate_dir + "/macro" + pol_str + ".csv", encoding="utf-8", header=True)
    cat_info.to_csv(intermediate_dir + "/cat_info" + pol_str + ".csv", encoding="utf-8", header=True)
    hazard_ratios.to_csv(intermediate_dir + "/hazard_ratios" + pol_str + ".csv", encoding="utf-8", header=True)
