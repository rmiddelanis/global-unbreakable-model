...
def process_input(pol_str, macro, cat_info, hazard_ratios, economy, event_level, default_rp, verbose_replace=True):
    ...
    macro["gdp_pc_pp"] = macro["avg_prod_k"] * agg_to_economy_level(cat_info, "k",
                                                                    economy)  # here we assume that gdp = consumption = prod_from_k
    cat_info["c"] = (1 - macro["tau_tax"]) * macro["avg_prod_k"] * cat_info["k"] + cat_info["gamma_SP"] * macro[
        "tau_tax"] * macro["avg_prod_k"] * agg_to_economy_level(cat_info, "k", economy)
    ...
    macro["macro_multiplier"] = (macro["avg_prod_k"] + recons_rate) / (
            macro["rho"] + recons_rate)  # Gamma in the technical paper

    ####FORMATING
    # gets the event level index
    event_level_index = hazard_ratios_event.reset_index().set_index(
        event_level).index  # index composed on countries, hazards and rps.
    ...
    cols = [c for c in macro_event if
            c in hazard_ratios_event]  # columns that are both in macro_event and hazard_ratios_event
    if not cols == []:
        if verbose_replace:
            flag1 = True
...
def calculate_response(macro_event, cats_event_ia, event_level, helped_cats, optionFee="tax", optionT="data",
                       optionPDS="unif_poor", optionB="data", loss_measure="dk", fraction_inside=1, share_insured=.25):
    ...
    if optionFee != "insurance_premium":
        macro_event, cats_event_iah = compute_response(macro_event, cats_event_iah, event_level, optionT=optionT,
    ...
    # special case of insurance that adds to existing default PDS
        ...
        m__, c__ = compute_response(macro_event, cats_event_iah, event_level, optionT="data", optionPDS="unif_poor",
                                    optionB="data", optionFee="tax", fraction_inside=1, loss_measure="dk")
        c__h = c__.rename(columns=dict(
            helped_cat="has_received_help_from_PDS_cat"))  # change column name helped_cat to has_received_help_from_PDS_cat
...
def compute_response(macro_event, cats_event_iah, event_level, optionT="data", optionPDS="unif_poor", optionB="data",
    ...
    macro_event["fa"] = agg_to_event_level(cats_event_iah, "fa",
                                           event_level) / 2  # because cats_event_ia is duplicated in cats_event_iah, cats_event_iah.n.groupby(level=event_level).sum() is 2 instead of 1, here /2 is to correct it. macro_event["fa"] =  agg_to_event_level(cats_event_ia,"fa") would work but needs to pass a new variable cats_event_ia.
    ...
    # print(cats_event_iah.n.groupby(level=event_level).sum())
    ...
    if optionPDS == "no":
        ...
        cats_event_iah.loc[(cats_event_iah.helped_cat == 'helped'), "help_needed"] = macro_event["shareable"] * \
                                                                                    cats_event_iah.loc[(
                                                                                                              cats_event_iah.helped_cat == 'helped') & (
                                                                                                              cats_event_iah.affected_cat == 'a') & (
                                                                                                              cats_event_iah.income_cat == 'poor'), loss_measure]

        cats_event_iah.loc[(cats_event_iah.helped_cat == 'not_helped'), "help_needed"] = 0
        ...
        cats_event_iah.loc[(cats_event_iah.helped_cat == 'helped'), "help_needed"] = macro_event["shareable"] * \
                                                                                    cats_event_iah.loc[(
                                                                                                              ...
                                                                                                              cats_event_iah.income_cat == 'poor'), loss_measure]
        cats_event_iah.loc[
            (cats_event_iah.helped_cat == 'not_helped') | (cats_event_iah.income_cat == 'nonpoor'), "help_received"] = 0
        ...
        if not "has_received_help_from_PDS_cat" in cats_event_iah.columns:
            cats_event_iah.loc[(cats_event_iah.helped_cat == 'helped'), "help_needed"] = macro_event["shareable"] * \
                                                                                        cats_event_iah.loc[(
                                                                                                                  cats_event_iah.helped_cat == 'helped') & (
                                                                                                                  cats_event_iah.affected_cat == 'a') & (
                                                                                                                  cats_event_iah.income_cat == 'nonpoor'), loss_measure]
            cats_event_iah.loc[
                (cats_event_iah.helped_cat == 'not_helped') | (cats_event_iah.income_cat == 'poor'), "help_needed"] = 0
        else:
            cats_event_iah.loc[(cats_event_iah.helped_cat == 'helped'), "help_needed"] = macro_event["shareable"] * \
                                                                                        cats_event_iah.loc[(
                                                                                                                  cats_event_iah.helped_cat == 'helped') & (
                                                                                                                  cats_event_iah.affected_cat == 'a') & (
                                                                                                                  cats_event_iah.income_cat == 'nonpoor') & (
                                                                                                                  cats_event_iah.has_received_help_from_PDS_cat == 'helped'), loss_measure]
            cats_event_iah.loc[
                (cats_event_iah.helped_cat == 'not_helped') | (cats_event_iah.income_cat == 'poor'), "help_needed"] = 0
        ...
        if not "has_received_help_from_PDS_cat" in cats_event_iah.columns:
            cats_event_iah.loc[
                (cats_event_iah.helped_cat == 'helped') & (cats_event_iah.income_cat == 'poor'), "help_needed"] = \
                macro_event["shareable"] * cats_event_iah.loc[
                    (cats_event_iah.helped_cat == 'helped') & (cats_event_iah.affected_cat == 'a') & (
                            cats_event_iah.income_cat == 'poor'), loss_measure]
            cats_event_iah.loc[
                (cats_event_iah.helped_cat == 'helped') & (cats_event_iah.income_cat == 'nonpoor'), "help_needed"] = \
                macro_event["shareable"] * cats_event_iah.loc[
                    (cats_event_iah.helped_cat == 'helped') & (cats_event_iah.affected_cat == 'a') & (
                            cats_event_iah.income_cat == 'nonpoor'), loss_measure]
            cats_event_iah.loc[cats_event_iah.helped_cat == 'not_helped', "help_needed"] = 0
            ...
            cats_event_iah.loc[
                ...
                macro_event["shareable"] * cats_event_iah.loc[
                    (cats_event_iah.helped_cat == 'helped') & (cats_event_iah.affected_cat == 'a') & (
                            cats_event_iah.income_cat == 'poor') & (
                            cats_event_iah.has_received_help_from_PDS_cat == 'helped'), loss_measure]
            cats_event_iah.loc[
                (cats_event_iah.helped_cat == 'helped') & (cats_event_iah.income_cat == 'nonpoor'), "help_needed"] = \
                macro_event["shareable"] * cats_event_iah.loc[
    ...
    if optionB == "data":
        macro_event["aid"] = (macro_event["need"] * macro_event["prepare_scaleup"] * macro_event["borrow_abi"]).clip(
        ...
        d = cats_event_iah.loc[(cats_event_iah.helped_cat == 'helped')]
        d["un"] = 1
        macro_event["need"] = agg_to_event_level(d, "un", event_level)
        macro_event["aid"] = macro_event["need"]
    elif optionB == "one":
    ...
    if optionPDS == "unif_poor":
        ...
        macro_event["unif_aid"] = macro_event["aid"] / (
            cats_event_iah.loc[(cats_event_iah.helped_cat == "helped") & (cats_event_iah.income_cat == 'poor'), "n"].groupby(level=event_level).sum())
        cats_event_iah.loc[(cats_event_iah.helped_cat == 'helped'), "help_received"] = macro_event["unif_aid"]
        cats_event_iah.loc[
            (cats_event_iah.helped_cat == 'not_helped') | (cats_event_iah.income_cat == 'nonpoor'), "help_received"] = 0
    elif optionPDS == "prop":
    ...
    if optionFee == "tax":
        cats_event_iah["help_fee"] = fraction_inside * macro_event["aid"] * cats_event_iah["k"] / agg_to_event_level(
            cats_event_iah, "k", event_level)
    elif optionFee == "insurance_premium":
        cats_event_iah.loc[(cats_event_iah.income_cat == 'poor'), "help_fee"] = fraction_inside * agg_to_event_level(
            cats_event_iah.query("income_cat=='poor'"), 'help_received', event_level) / (cats_event_iah.query(
            "income_cat=='poor'").n.sum())
        cats_event_iah.loc[(cats_event_iah.income_cat == 'nonpoor'), "help_fee"] = fraction_inside * agg_to_event_level(
            cats_event_iah.query("income_cat=='nonpoor'"), 'help_received', event_level) / (cats_event_iah.query(
...
def process_output(macro, out, macro_event, economy, default_rp, return_iah=True, is_local_welfare=True):
    ...
    for i in ['axfin', 'social', 'gamma_SP']:
        dkdw[i] = dkdw_h[i].groupby(level=economy).mean()
...
def interpolate_rps(fa_ratios, protection_list, option):
    ...
    if type(fa_ratios.columns) == pd.MultiIndex:
        ...
        return pd.concat({col: interpolate_rps(fa_ratios[col], protection_list, option) for col in keys}, axis=1).stack(
            "rp")
    ...
    fa_ratios_rps = pd.concat(
        ...
        , axis=1).sort_index(axis=1).clip(lower=0).ffill(axis=1)
...
def calc_delta_welfare(micro, macro):
    ...
    dw = welf(micro["c"] / macro["rho"], macro["income_elast"]) - welf(
        micro["c"] / macro["rho"] - (micro["dc_npv_post"]), macro["income_elast"])
...
def average_over_rp(df, default_rp, protection=None):
    ...
    averaged = df.mul(proba_serie, axis=0).groupby(level=idxlevels).sum()  # frequency times each variables in the columns including rp.
