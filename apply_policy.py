...
def apply_policy(macro_, cat_info_, hazard_ratios_, policy_name=None, policy_opt=None, verbose=True):
    ...
    if policy_name in [None, 'baseline']:
        ...
        policy_name = 'reduce_poor_exposure_{}'.format(int(policy_opt * 100))
        ...
        policy_name = 'reduce_rich_exposure_{}'.format(int(policy_opt * 100))
        ...
        policy_name = 'reduce_nonpoor_exposure_{}'.format(int(policy_opt * 100))
        ...
        policy_name = 'reduce_total_exposure_{}'.format(int(policy_opt * 100))
    elif policy_name == 'no_liquidity':
        desc = "No liquidity"
        cat_info['liquidity'] = 0
    elif policy_name == 'reduce_ew':
        desc = "Reduce early warning to {policy_opt * 100}%"
        macro.ew = macro.ew * policy_opt
        hazard_ratios.ew = hazard_ratios.ew * policy_opt
        policy_name = 'reduce_ew_{}'.format(int(policy_opt * 100))
