...
def get_dk_over_k_from_file(path='inputs/GAR_data_surge.csv'):
    ...
    gardata_surge = gardata_surge.set_index(replace_with_warning(gardata_surge.Country, iso3_to_wb))[
        ['AAL', 'Exposed Value (from GED)']]

    return gardata_surge['AAL'].groupby(level=0).sum() / gardata_surge['Exposed Value (from GED)'].groupby(level=0).sum()
...
def gar_preprocessing(inputs, intermediates):
    ...
    gardata = pd.read_csv('inputs/GAR15 results feb 2016_PML mundo.csv', encoding='latin-1', header=[0, 1, 2],
    ...
    # frac_cap_distroyed_from_events = capital_losses_from_GAR_events/ev_gar.groupby(level="country").median()
    ...
    paf = (AAL_splitted / average_over_rp(frac_value_destroyed_gar).squeeze()).replace(np.inf, np.nan).sort_values(
        ascending=False)
    ...
    test = frac_value_destroyed_gar_completed.unstack().replace(0, np.nan).dropna().assign(
        test=lambda x: x[new_rp] / x[1500]).test
    # print(frac_value_destroyed_gar_completed["United States"])

    pd.DataFrame(
        (average_over_rp(frac_value_destroyed_gar).squeeze() / AAL_splitted).replace(0, np.nan).dropna().sort_values())

    print('GAR preprocessing script: writing out intermediate/frac_value_destroyed_gar_completed.csv')
    frac_value_destroyed_gar_completed.to_csv("intermediate/frac_value_destroyed_gar_completed.csv", encoding="utf-8",
                                              header=True)
