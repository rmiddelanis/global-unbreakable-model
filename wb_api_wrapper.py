...
def get_wb(wb_name):
    ...
    # return wb.download(indicator=wb_name, start=start_year, end=today_year, country="all").unstack("year")[
    #     wb_name].dropna(how="all").dropna(how="all", axis=1)
    """return cleaned (removed nan) dataframe (countries, year, data) with WB data"""
    res = wb.download(indicator=wb_name, start=start_year, end=today_year, country="all")
