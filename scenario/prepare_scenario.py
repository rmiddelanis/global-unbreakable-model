...
def estimate_real_est_k_to_va_shares_ratio(root_dir, any_to_wb, verbose):
    ...
    # estat_capital_sector = pd.read_csv(os.path.join(root_dir, "data/raw/Eurostat/eurostat__nama_10_nfa_bs__capital_stock.csv"))[['sector', 'asset10', 'geo', 'TIME_PERIOD', 'OBS_VALUE']]
...
def calc_asset_shares(root_dir_, any_to_wb_, scale_self_employment=None,
    ...
    if not force_recompute and os.path.exists(capital_shares_before_adjustment_path):
        ...
        # labor_share_of_gdp = pd.read_csv(os.path.join(root_dir_, "data/raw/", labor_share_data_file))
...
def prepare_scenario(scenario_params):
    scenario_root_dir = os.path.dirname(os.path.abspath(__file__))
    any_to_wb, iso3_to_wb, iso2_iso3 = get_country_name_dicts(scenario_root_dir)
